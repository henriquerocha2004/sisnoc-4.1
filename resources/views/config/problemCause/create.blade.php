                ...
                <div class="col-md-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <strong>Cadastro </strong> de Causas de Problemas
                            <small style="color:red" class="text-right"><i>*</i> Campos Obrigatórios</small>
                        </div>
                        <div class="card-body card-block">
                            <form action="{{route('cause-problem.store')}}" method="post" class="" autocomplete="off">
                                @csrf
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="description_cause" class=" form-control-label">Descrição<i style="color:red">*</i></label>
                                            <input required type="description_cause" id="description_cause" name="description_cause" value="{{old('description_cause')}}" class="form-control {{ ($errors->has('description_cause') ? 'is-invalid': '') }}"">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select required  name="id_category" id="id_category" class="form-control {{ ($errors->has('id_category') ? 'is-invalid': '') }}">
                                                ...
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}">{{ $category->description_category }}</option>
                                                @endforeach
                                            </select>
