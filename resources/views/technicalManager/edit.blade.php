...
@section('title')
    <title>Sisnoc | Editar Técnicos</title>
@endsection
                ...
                <div class="col-md-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <strong>Editar </strong>  Responsável Técnico
                            <small style="color:red" class="text-right"><i>*</i> Campos Obrigatórios</small>
                        </div>
                        <div class="card-body card-block">
                            <form action="{{route('technicalManager.update', $technicalManager->id)}}" method="post" class="" autocomplete="off">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input  type="text" id="name" name="name" value="{{old('name') ?? $technicalManager->name}}" class="form-control {{ ($errors->has('name') ? 'is-invalid': '') }}"">
                                            @if($errors->has('name'))
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input  type="text" value="{{old('contact') ?? $technicalManager->contact}}" placeholder="(xx) xxxxx-xxxx" id="contact" name="contact"  class="form-control {{ ($errors->has('contact') ? 'is-invalid': '') }}">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input  type="email" value="{{old('email') ?? $technicalManager->email}}" id="email" name="email" class="form-control {{ ($errors->has('email') ? 'is-invalid': '') }}">
                                            @if($errors->has('email'))
                                                @component('compoments.feedbackInputs', ['typeFeed' => 'invalid'])
                                                    {{$errors->first('email')}}
                                                @endcomponent
                                ...
                                <div class="row">
                                    <div class="col-md-12 mt-4">
                                        ...
                                        <input type="hidden" name="selected_establishment" value="{{old('selected_establishment') ?? count($technicalManager->idEstablishments()['ids'])}}">
                                        <div class="row">
                                            <div class="col col-md-9 mt-2">
                                                <select name="establishment_code[]" id="establishment_code" multiple="" class="form-control {{ ($errors->has('selected_establishment') ? 'is-invalid': '') }}">
                                                    @foreach ($establishments as $establishment)
                                                        @if (old('establishment_code') || $technicalManager->establishments()->get())
                                                            @foreach (old('establishment_code') ?? $technicalManager->idEstablishments()['ids'] as $code)
                                                                @php
